...
function PracticeContent(){
  ...
  const latestCompleted=(id:string)=>attempts.filter(a=>a.testId===id && !!a.completedAt).sort((a,b)=>new Date(b.completedAt||b.updatedAt).getTime()-new Date(a.completedAt||a.updatedAt).getTime())[0];
  const tipsForPillar=(id:string)=>{
    const tips:Record<string,string[]>={
      deductive:["Translate every rule into a clear condition before evaluating the options.","Separate what must be true from what could be true.","Do not add assumptions that are not stated in the question."],
      logical:["Look for the underlying rule before comparing individual options.","Track sequences and relationships systematically rather than by intuition.","Test your rule against every given example before choosing."],
      diagrammatic:["Scan the whole diagram first, then isolate the changing elements.","Track position, rotation, count and shape systematically.","Compare consecutive steps to identify the smallest consistent rule."],
      numerical:["Identify exactly what each number represents before calculating.","Estimate first so you can catch implausible results.","Keep units and percentage changes consistent throughout the calculation."],
      verbal:["Answer only from the passage; do not use outside knowledge.","Watch for absolute wording that the text does not fully support.","If the passage never covers a claim, choose insufficient or cannot say."],
      situational:["Choose the response that addresses the issue constructively and professionally.","Consider the impact on people, customers and the wider team.","Prefer proportionate action and clear communication over unnecessary escalation."]
    };
    return tips[id]||["Read the question carefully before looking at the answer choices.","Use only the information provided in the test.","If two options seem close, return to the exact wording and test each one."] 
  };
  ...
  return <div className="app-page practice-library-page">
    ...
    {visibleTests.length?<div className="test-library-grid">
      {visibleTests.map((t:any,index:number)=>{
        const qs=questionsForTest(t.test_id); const a=latest(t.test_id); const completedAttempt=latestCompleted(t.test_id); const starred=stars.includes(t.test_id); const absoluteIndex=(safePage-1)*pageSize+index; const tone=tones[absoluteIndex%tones.length];
        return <article key={t.test_id} className={`test-library-card ${tone}`} onClick={()=>setLaunchTest(t)}>
          ...
          <div className="test-card-body">
            <div className="test-card-topline"><span className="practice-index">TEST {t.test_id.replace("TEST_","")}</span><button type="button" className={`test-star ${starred?"starred":""}`} onClick={(e)=>{e.stopPropagation();toggleStarredTest(t.test_id);setStars(getStarredTests())}} aria-label={starred?"Remove bookmark":"Bookmark test"}>{starred?"★":"☆"}</button></div>
            ...
            <button type="button" className="test-start" onClick={(e)=>{e.stopPropagation();setLaunchTest(t)}}>{a?.completedAt?"Review test":a?"Continue test":"Start"} <span>→</span></button>
          </div>
        </article>
      })}
    ...
    {launchTest&&(()=>{
      const modalQuestions=questionsForTest(launchTest.test_id);
      const completed=latestCompleted(launchTest.test_id);
      const inProgress=latest(launchTest.test_id);
      const result=completed?modalQuestions.reduce((sum:number,q:any)=>sum+(answersMatch(q,completed.answers?.[q.id])?1:0),0):0;
      const accuracy=completed&&modalQuestions.length?Math.round(result/modalQuestions.length*100):0;
      const tips=tipsForPillar(testPillar(launchTest));
      return <div className="launch-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)setLaunchTest(null)}}>
        <div className="launch-modal" role="dialog" aria-modal="true" aria-labelledby="launch-title">
          ...
          <div className="launch-meta"><span>{modalQuestions.length} questions</span><span>{modalQuestions.length} minutes</span><span>Timed</span></div>
          <section className="launch-tips"><h3>Quick tips</h3><ul>{tips.map((tip:string,i:number)=><li key={i}>{tip}</li>)}</ul></section>
          {completed?<section className="launch-result"><div><span>Latest result</span><strong>{result} / {modalQuestions.length}</strong></div><div><span>Accuracy</span><strong>{accuracy}%</strong></div><div><span>Time</span><strong>{Math.floor((completed.durationSeconds||0)/60)}m {(completed.durationSeconds||0)%60}s</strong></div></section>:<p className="launch-copy">{inProgress?"You have an unfinished attempt. Continue where you left off.":"Ready to begin? Your answers and progress will be saved as you work through the test."}</p>}
          <div className="launch-actions"><button type="button" className="outline-action" onClick={()=>setLaunchTest(null)}>Not yet</button>{completed?<><button type="button" className="outline-action" onClick={()=>router.push(`/tests/${launchTest.test_id}/result`)}>Review result →</button><button type="button" className="yellow-button" onClick={()=>{window.localStorage.setItem("aptitude-practice-last-pillar",activePillar);window.localStorage.setItem(pageStorageKey,String(safePage));window.localStorage.setItem("aptitude-practice-return-page",String(safePage));router.push(`/tests/${launchTest.test_id}`)}}>Retake test →</button></>:<button type="button" className="yellow-button" onClick={()=>{window.localStorage.setItem("aptitude-practice-last-pillar",activePillar);window.localStorage.setItem(pageStorageKey,String(safePage));window.localStorage.setItem("aptitude-practice-return-page",String(safePage));router.push(`/tests/${launchTest.test_id}`)}}>{inProgress?"Continue test →":"Start test →"}</button>}</div>
        </div>
      </div>
    })()}
    ...
    <style jsx>{`
      ...
      .launch-overlay{position:fixed;inset:0;z-index:100;display:grid;place-items:center;padding:24px;background:rgba(25,25,22,.62)}.launch-modal{position:relative;width:min(520px,100%);padding:26px;border:1px solid #dedbd3;border-radius:18px;background:#fffefa;box-shadow:0 24px 80px rgba(28,27,24,.18)}.launch-close{position:absolute;right:17px;top:14px;border:0;background:transparent;font-size:24px;line-height:1;color:#8d8981}.launch-art{height:92px;margin-bottom:22px;border:1px solid rgba(34,35,33,.12);display:grid;place-items:center;background:var(--pink);overflow:hidden;position:relative}.launch-art:before,.launch-art:after{content:"";position:absolute;border:1px solid rgba(34,35,33,.35);border-radius:50%;width:54px;height:54px}.launch-art:after{transform:translateX(24px)}.launch-art span{position:relative;z-index:2;font-size:30px}.launch-modal h2{margin-top:8px;font-size:32px;line-height:1;letter-spacing:-.05em;font-weight:500}.launch-meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:17px}.launch-meta span{padding:7px 10px;border-radius:999px;background:#f2f1ec;font-size:10px;font-weight:700;color:#69665f}.launch-copy{margin-top:18px;max-width:430px;font-size:12px;line-height:1.7;color:#77736b}.launch-tips{margin-top:18px;padding:14px 16px;border:1px solid var(--line);background:#fbfaf6}.launch-tips h3{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#5f5d58}.launch-tips ul{margin:9px 0 0;padding-left:18px;color:#77736b;font-size:11px;line-height:1.6}.launch-tips li+li{margin-top:4px}.launch-result{display:grid;grid-template-columns:1.2fr 1fr 1fr;gap:10px;margin-top:18px;padding:14px 15px;border:1px solid var(--line);background:#fbfaf6}.launch-result div{display:flex;flex-direction:column;gap:4px}.launch-result span{font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#aaa7a0}.launch-result strong{font-size:17px;font-weight:600;color:#2b2b28}.launch-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:24px}.launch-actions .yellow-button{border:0}.launch-actions .outline-action{background:#fff}
