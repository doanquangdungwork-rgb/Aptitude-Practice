...
import { completeAttempt, getAttempt, getBookmarks, getCompletedAttempt, getWrongQuestions, saveAttempt, setWrongQuestions, toggleBookmark } from "../../../lib/progress";
...
export default function TestPage() {
  ...
  const finish=()=>{const wrong=qs.filter((x:any)=>answers[x.id]&&String(answers[x.id]).trim().toLowerCase()!==String(x.a??"").trim().toLowerCase()).map((x:any)=>x.id);setWrongQuestions([...new Set([...getWrongQuestions(),...wrong])]);const seconds=Math.max(0,Math.round((Date.now()-new Date(startedAt).getTime())/1000));completeAttempt(attemptId,answers,seconds);router.push(`/tests/${testId}/result`);};
