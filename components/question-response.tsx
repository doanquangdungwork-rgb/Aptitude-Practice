...
export default function QuestionResponse({response,options,answer,value,onChange,screenshotMode=false}:{response:CanonicalResponse;options:CanonicalOption[];answer?:CanonicalAnswer;value:unknown;onChange:(value:unknown)=>void;screenshotMode?:boolean}){
 ...
 const labelContent=(o:CanonicalOption,i:number)=>screenshotMode ? (isFigureOption(o) ? <span>{o.content.value}</span> : <span>{o.id}</span>) : <QuestionPrompt blocks={[o.content]}/>;
 ...
 if(response.type==="multiple_choice") return <div className="mt-8 grid gap-3">{requiredSelections ? <div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#77756e]">Choose {requiredSelections} {requiredSelections===1?"answer":"answers"}</div> : null}{options.map((o,i)=>{const active=selected.includes(o.id);const atLimit=Boolean(requiredSelections && !active && selected.length>=requiredSelections);return <button key={o.id} type="button" aria-pressed={active} disabled={atLimit} onClick={()=>onChange(active?selected.filter(x=>x!==o.id):[...selected,o.id])} className={`quiz-option text-left ${active?"selected":""} ${atLimit?"cursor-not-allowed opacity-50":""}`}><span className="mr-3 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#f2f1eb] text-xs font-bold">{isFigureOption(o) ? o.content.value : o.id}</span>{labelContent(o,i)}</button>})}</div>;
