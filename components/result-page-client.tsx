...
const referenceMaterialsForQuestion = (testId: string, n: number): ReferenceMaterial[] => {
  if (testId === "TEST_001" && n >= 1 && n <= 12) return [CAPP_UNIQUE_CHARTS[Math.min(3, Math.floor((n - 1) / 3))]];
  ...
  if (DEDUCTIVE_REFERENCES[testId] && (n === 5 || n === 6)) return [DEDUCTIVE_REFERENCES[testId][0]];
  if (DEDUCTIVE_REFERENCES[testId] && (n === 17 || n === 18)) return [DEDUCTIVE_REFERENCES[testId][1]];
  return [];
};

const optionForAnswer = (q: any, value: unknown) => {
  const raw = String(value ?? "");
  return q.options?.find((o: any, i: number) => normalizeChoice(o.id) === normalizeChoice(raw) || String.fromCharCode(65 + i) === raw.toUpperCase());
};
const answerLabel = (q: any, value: unknown) => {
  if (q.answer?.type === "composite" && value && typeof value === "object" && !Array.isArray(value)) {
    const parts = value as Record<string, unknown>;
    return `Most: ${String(parts.most ?? "")} · Least: ${String(parts.least ?? "")}`;
  }
  if (q.answer?.type === "multiple") {
    const values = Array.isArray(value) ? value : q.answer.values;
    return values.map((v: unknown) => String(v)).join(", ");
  }
  const option = optionForAnswer(q, value);
  if (option) return option.id;
  return String(value ?? "");
};
const correctLabel = (q: any) => {
  if (q.answer?.type === "composite") {
    return `Most: ${String(q.answer.parts?.most ?? "")} · Least: ${String(q.answer.parts?.least ?? "")}`;
  }
  if (q.answer?.type === "multiple") return q.answer.values.map((v: unknown) => String(v)).join(", ");
  const option = optionForAnswer(q, q.answer?.value);
  return option?.id ?? String(q.answer?.value ?? "");
};
...
export default function ResultPageClient({ testId, test, qs, pillarMap }: { testId: string; test: any; qs: any[]; pillarMap: Record<string, string> }) {
  ...
  const selectedSnapshotSrc = selectedQuestion ? questionSnapshotSrc(testId, selectedQuestion.source?.sourceFile, selectedQuestion.number) : "";
  const selectedSolutionSrc = selectedQuestion ? solutionSnapshotSrc(testId, selectedQuestion.source?.sourceFile, selectedQuestion.number) : "";
  ...
  return <div className="app-page quiz-page review-page">
    ...
    <div className="review-workspace">
      <section className="review-reference-pane">
        {selectedSnapshotSrc ? <QuestionSnapshot questionNumber={selectedQuestion.number} src={selectedSnapshotSrc} solutionSrc={selectedSolutionSrc} showSolution={Boolean(selectedSolutionSrc)} fallback={selectedReferenceFallback} /> : selectedReferenceFallback}
      </section>
      <section className="review-question-pane">
        <QuestionNavigator count={qs.length} current={selectedIndex} getStatus={(i) => statusFor(qs[i])} onSelect={(i) => { setSelectedIndex(i); document.getElementById(`question-${qs[i].number}`)?.scrollIntoView({ behavior: "smooth", block: "start" }); }} label="Answers" />
          ...
          {qs.map((q, i) => {
            ...
            return <article key={q.id} id={`question-${q.number}`} onClick={() => setSelectedIndex(i)} className={`review-item ${selectedIndex === i ? "review-item-active" : ""} ${skipped ? "skipped" : isCorrect ? "correct" : "wrong"}`}>
              ...
              {q.options.length > 0 && q.response.type === "composite" ? (
                <div className="mt-5 grid gap-5">
                  {["most", "least"].map((part) => {
                    ...
                    return <div key={part}>
                      ...
                      <div className="grid gap-2">{q.options.map((o: any, j: number) => {
                ...
                <div className="mt-5 grid gap-2">{q.options.map((o: any, j: number) => {
                  const selectedHere = Array.isArray(selected) ? selected.includes(o.id) : String(selected ?? "") === o.id;
                  const correctHere = q.answer.type === "single" ? normalizeChoice(o.id) === normalizeChoice(q.answer.value) : q.answer.type === "multiple" && q.answer.values.some((v: unknown) => normalizeChoice(v) === normalizeChoice(o.id));
                  return <div key={o.id} className={`review-option ${correctHere ? "correct-answer" : selectedHere ? "selected-wrong" : ""}`}><span className="mr-2 font-bold">{String.fromCharCode(65 + j)}.</span><QuestionPrompt blocks={[o.content]}/>{correctHere && <span className="ml-2 text-xs">✓ Correct answer</span>}{selectedHere && !correctHere && <span className="ml-2 text-xs">Your answer</span>}</div>;
                })}</div>
              ) : null}

              {skipped && <div className="mt-5 rounded-xl border border-[#e7e5de] bg-[#fbfaf6] p-4 text-sm"><b>Correct answer:</b> {correctLabel(q) || "Key not mapped"}</div>}
              {!skipped && !isCorrect && <div className="mt-5 rounded-xl bg-[#fff8d8] p-4 text-sm"><b>Your answer:</b> {answerLabel(q, selected)}<br /><b>Correct answer:</b> {correctLabel(q) || "Key not mapped"}</div>}
              {q.explanation && <div className="mt-5 border-t border-current/10 pt-4 text-sm leading-6 text-[#6f6b64]"><b>Explanation</b><div className="mt-1">{q.explanation.blocks.map((b: any, j: number) => b.type === "text" ? <span key={j} className="whitespace-pre-wrap">{b.value}</span> : null)}</div></div>}
