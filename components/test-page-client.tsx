...
import { completeAttempt, getAttempt, getBookmarks, getWrongQuestions, recordPracticeDay, saveAttempt, setWrongQuestions, toggleBookmark, syncAttemptToSupabase } from "../lib/progress";
...
export default function TestPageClient({ testId, test, qs, pillarMap }: { testId: string; test: any; qs: any[]; pillarMap: Record<string, string> }) {
  ...
  const submitFinish = async () => {
    ...
    const completedAttempt = { id: attemptId, testId, startedAt, updatedAt: new Date().toISOString(), completedAt: new Date().toISOString(), durationSeconds: Math.min(seconds, testDurationSeconds || seconds), answers };
    completeAttempt(attemptId, answers, Math.min(seconds, testDurationSeconds || seconds));
    const session = await supabase?.auth.getSession();
    if (session?.data.session?.user?.id) await syncAttemptToSupabase(completedAttempt, qs, test.pillar);
