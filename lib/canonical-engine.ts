...
export type CanonicalQuestion = {
  ...
  explanation?: { blocks: ContentBlock[] } | null;
...
export function answersMatch(question: CanonicalQuestion, selected: unknown): boolean {
  ...
  if (answer.type === "numeric") { const actual = Number(selected); const expected = Number(answer.value); if (!Number.isFinite(actual) || !Number.isFinite(expected)) return normalizeChoice(selected) === normalizeChoice(answer.value); return Math.abs(actual - expected) <= (answer.tolerance ?? 0); }
