...
export function referenceMaterialsForQuestion(testId: string, questionNumber: number): ReferenceMaterial[] {
  ...
  if (testId === "TEST_030" && (questionNumber === 5 || questionNumber === 6)) return [{ id: "DED1_BROADBAND", assetRef: "TEST_030_P06", label: "Broadband plans" }];
  if (testId === "TEST_030" && (questionNumber === 17 || questionNumber === 18)) return [{ id: "DED1_CONTRACTS", assetRef: "TEST_030_P18", label: "Salaries & contracts" }];
...
function inferAnswer(q: LegacyQuestion, response: CanonicalQuestion["response"], options: { id: string; content: ContentBlock }[]): CanonicalQuestion["answer"] { const raw = q.a; const rawAnswer = String(raw ?? "").trim(); const text = String(q.t ?? ""); const explanation = String(q.explanation ?? ""); if (response.type === "numeric") return { type: "numeric", value: rawAnswer }; if (isTrueFalseCannotSay(text)) { const answerMatch = explanation.match(/correct answer is\s+(true|false|cannot say)/i); return { type: "single", value: (answerMatch?.[1] ?? rawAnswer).trim().toLowerCase() }; } if (response.type === "text") return { type: "text", value: rawAnswer }; if (response.type === "multiple_choice") { const values = Array.isArray(raw) ? raw.map(String) : rawAnswer.split(/[,|]/).map(v => v.trim()).filter(Boolean); return { type: "multiple", values }; } const matchedOption = options.find(option => option.id.toLowerCase() === rawAnswer.toLowerCase()); return { type: "single", value: matchedOption?.id ?? rawAnswer }; }
...
function deductiveQuestion(q: DeductiveSourceQuestion): CanonicalQuestion { const options = q.o.map((value, index) => ({ id: String.fromCharCode(65 + index), content: legacyBlock(value) })); const answer = options.find(o => o.content.type === "text" && o.content.value === q.a)?.id ?? q.a; return { id: q.id, number: q.number, context: q.context, prompt: { blocks: legacyBlocks(q.t) }, options, response: { type: "single_choice" }, answer: { type: "single", value: answer }, taxonomy: { pillar: q.p, subtype: q.s }, source: { sourceFile: q.sourceFile, sourcePage: q.sourcePage } }; }
...
const canonicalOverrides = new Map<string, CanonicalTest>(); const test001 = normalizeCanonicalTest(canonicalTest001); if (test001) canonicalOverrides.set(test001.id, test001);
const deductiveTest: CanonicalTest = { id: "TEST_030", title: "DeductiveTest1", taxonomy: { pillar: "logical", subtype: "deductive_reasoning" }, timing: { mode: "none" }, questions: (deductiveTest1 as DeductiveSourceQuestion[]).map(deductiveQuestion) }; canonicalOverrides.set("TEST_030", deductiveTest);
const grouped = new Map<string, LegacyQuestion[]>(); for (const rawQuestion of allQuestions as LegacyQuestion[]) { const id = String(rawQuestion.testId ?? rawQuestion.id.split("_")[0]); const list = grouped.get(id) ?? []; list.push(rawQuestion); grouped.set(id, list); }
