...
function deductiveQuestion(q: DeductiveSourceQuestion): CanonicalQuestion {
  const options = q.o.map((value, index) => ({ id: String.fromCharCode(65 + index), content: legacyBlock(value) }));
  const answer = options.find(o => o.content.type === "text" && o.content.value === q.a)?.id ?? q.a;
  return { id: q.id, number: q.number, context: q.context, prompt: { blocks: legacyBlocks(q.t) }, options, response: { type: "single_choice" }, answer: { type: "single", value: answer }, taxonomy: { pillar: q.p, subtype: q.s }, source: { sourceFile: q.sourceFile, sourcePage: q.sourcePage } };
}
